...
public class ReplaceAreaWithToleranceActivity extends ActionBarActivity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		setContentView(R.layout.activity_replace_area_with_tolerance);
		...
		imageView.setOnTouchListener(new View.OnTouchListener()
		{
			...
			public boolean onTouch(View v, final MotionEvent event)
			{
				...
				if (action == MotionEvent.ACTION_DOWN)
				{
					...
					final ProgressDialog dialog = new ProgressDialog(ReplaceAreaWithToleranceActivity.this);
					...
					new Thread(new Runnable()
					{
						...
						public void run()
						{
							...
							final TypedArray styledAttributes = ReplaceAreaWithToleranceActivity.this.getTheme().obtainStyledAttributes(
							...
							int y = (int) event.getY() + actionBarSize;

							int realX = bitmap.getWidth() * x / width;
							int realY = bitmap.getHeight() * y / height;

							final Bitmap changedBitmap = ImageColorService.replaceAreaWithTolerance(bitmap, realX, realY, replacingColor, 20);
